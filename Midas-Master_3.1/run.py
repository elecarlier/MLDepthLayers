        ...
        for index, image_name in enumerate(image_names):
            ...
            if output_path is not None:
                filename = os.path.join(
                    output_path, os.path.splitext(os.path.basename(image_name))[0] + '_Map' 
                )
                utils.write_depth(filename, prediction, grayscale, bits=2)
                #
                # Cree la version Looking Glass si --side a ete precise
                #
                if side:                    
                    filename = os.path.join(
                        output_path, os.path.splitext(os.path.basename(image_name))[0] + '_lkg' 
                    )
                    original_image_bgr = np.flip(original_image_rgb, 2)
                    content = create_side_by_side(original_image_bgr*255, prediction, grayscale)
                    utils.write_depth(filename, content, grayscale, bits=2)
